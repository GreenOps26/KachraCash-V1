...
## 0. Implementation Status & Document Hierarchy

| Document | Scope |
|----------|-------|
| **This file** (`docs/AGENTS.md`) | All coding rules: product guardrails, SvelteKit tooling, financial integrity |
| [`PRD.md`](./PRD.md) | What to build |
| [`PHASES.md`](./PHASES.md) | When to build it |
| [`ARCHITECTURE.md`](./ARCHITECTURE.md) | How systems connect (target layout) |
| [`TDD.md`](./TDD.md) | How to verify |
| [`DESIGN.md`](./DESIGN.md) | UX and design system |

Non-builder docs (pitch deck, feature matrix archive, HTML mocks) live in [`../archive/docs/`](../archive/docs/).

**Current repo:** pnpm + Turborepo monorepo. Admin is SvelteKit (`/apps/admin`); citizen and partner are Expo (`/apps/citizen`, `/apps/partner`). Shared packages: `@kachracash/types`, `@kachracash/ui`, `@kachracash/db`, `@kachracash/api`. Run `pnpm dev` from the repo root; `pnpm test:unit` for TDD unit tests; set `DATABASE_URL` for integration tests.
...
3. **Run Unit & Integration Test Suite:**
   ```bash
   npm run test          # Current repo
   pnpm run test         # Target monorepo
   ```
...
## 6. Current Repo: SvelteKit 5 Web Scaffold
...
### 6.2 Svelte MCP Tools

Use the Svelte MCP server for Svelte 5 and SvelteKit work:

1. **`list-sections`** — Discover documentation sections. Use FIRST for any Svelte/SvelteKit task.
2. **`get-documentation`** — Fetch full content for relevant sections after `list-sections`.
3. **`svelte-autofixer`** — Analyze Svelte code before shipping. Re-run until clean.
4. **`playground-link`** — Only after user confirmation; never when code was written to project files.
