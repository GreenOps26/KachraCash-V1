...
| Layer | Status | Notes |
|-------|--------|-------|
| **Monorepo** | pnpm + Turborepo with `/apps/*` and `/packages/*` | `pnpm dev` from repo root |
| **Admin** | SvelteKit 5 at `/apps/admin` | `pnpm --filter @kachracash/admin dev` (port 5173) |
| **Mobile** | Expo SDK 57 at `/apps/citizen`, `/apps/partner` | Ports 8081 / 8082; import `@kachracash/ui` |
| **Shared UI** | `/packages/ui` — Glade tokens + RN components | CSS copies for admin Tailwind |
| **Shared types** | `/packages/types` — DTOs, BLE stubs | Imported by all apps |
| **Database** | Prisma schema in `packages/db` | Run `pnpm db:generate`; migrations via `pnpm --filter @kachracash/db db:migrate` |
| **API layer** | `packages/api` Fastify server (`pnpm dev:api`) | `GET /api/v1/rates`, `POST /api/v1/pickups`, `POST /api/v1/orders/:id/complete`, admin ward suspend |

> When implementing, treat this document as the **target architecture**. See [`PHASES.md`](./PHASES.md) §Phase 1 implementation note.
...
│   └── admin/                    # SvelteKit 5 admin console (web)
├── packages/
│   ├── ui/                       # Glade tokens, CSS, and React Native components
...
## 2. Relational Schema (PostgreSQL / Neon)

The database layer targets **PostgreSQL 16 on Neon** (region: `ap-south-1`) with the **PostGIS** extension enabled (`CREATE EXTENSION IF NOT EXISTS postgis;`). Schema definitions will reside in `packages/db/prisma/schema.prisma` once the monorepo is scaffolded.

### 2.1 Domain Tables (14 core + 1 audit)

The schema defines **15 Prisma models**: 14 domain tables plus `AuditLog` for admin action tracing. `Rating` is included in the 14 domain tables.
...
DATABASE_URL="postgres://user:pass@ep-cool-lake-123456-pooler.ap-south-1.aws.neon.tech/kachracash?sslmode=require&pgbouncer=true"

# Direct unpooled connection string for migrations & DDL
DATABASE_URL_UNPOOLED="postgres://user:pass@ep-cool-lake-123456.ap-south-1.aws.neon.tech/kachracash?sslmode=require"
```
